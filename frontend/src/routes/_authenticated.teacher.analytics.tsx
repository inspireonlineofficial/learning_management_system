...
function Page() {
  ...
  if (pathname.startsWith("/teacher/analytics/students/")) {
    return <Outlet />;
  }
  ...
  return (
    <AppShell eyebrow="Analytics" title="Teacher analytics">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Students" value={data?.total_users ?? "—"} />
      ...
      <SectionHeading title="Activity (last 30 days)" />
        ...
        <div className="border border-brand/10 bg-white/40 p-6">
          ...
          <div className="mt-4 flex gap-6 text-xs text-brand/55">
            <span className="flex items-center gap-2">
              <span className="h-2.5 w-2.5 bg-accent/70" /> Active users
            </span>
            <span className="flex items-center gap-2">
